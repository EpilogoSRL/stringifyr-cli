...
export type TPathStaticSegment<A extends string, Result = A> = A extends `{${string}`
  ? never
  : A extends `${string}.${string}`
    ? never
    : Result;

export type TPathVariableSegment<A extends string, Result = A> = A extends `{${string}=${string}}`
  ? Result
  : never

export type TPathToNode = string;
